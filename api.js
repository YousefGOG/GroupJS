...
let message;
try { message = JSON.parse(msg) } catch { c.close() }

if(message.event === "request-hsw") {
queue[message.data.token] = c;
Array.from(wss.clients).forEach(async c =>{ c.send(JSON.stringify(message)) })
}else
if(message.event === "new-hsw") {
let client = queue[message.data.token]
if(client) {
delete queue[message.data.token]
client.send(JSON.stringify(message))
setTimeout(() => { try { client.close() } catch {} }, 3000)
}else{
Array.from(wss.clients).forEach(async c =>{ c.send(JSON.stringify(message)) })
}
...
console.log('r')
  response.sendFile(__dirname + "/htmlhsw.html");
});
...
setInterval(async () => {
    try {
let daaaaaaa = fs.readFileSync('./captchas.txt', {encoding:'utf8', flag:'r'})

let Array = daaaaaaa.split('\n')

console.log(Array.length)
if(Array.length > 300) return console.log("end")
   const response = await hcaptcha('https://captcha-protected-site.com');
console.log(response)
let da = daaaaaaa + "\n" + `${response}`

fs.writeFile("./captchas.txt", da, function(err) {if(err) console.log(err)});
...
/*
ODczMDU1MzU1NDY1MjU3MDAw.YQy2Kw.EIUBvFZ2flFkv7gkJanfjISTGQ8
ODczMDU1MzczMzY2NTM4Mjcy.YQy2Lw.aEEtO-GyA01rbnw66DHxyjNFzOA
ODczMDU1Mzc4NzIyNjg0OTU5.YQy2MA.ywxnu2GIAvEAWoSKL00LBVVUL0s
ODczMDU1MzgwNjEwMTA5NDcx.YQy2MQ.tZr6kJAl9LrkH7V864gXVl1DWVM
ODczMDU1MzkxMzU1OTA0MDUx.YQy2Mw.0_7PyfCw0UZyTH4NrqwJcD2fEnk
ODczMDU1Mzk1MjIzMDQ0MDk2.YQy2NA.73a1rC4a6xZdecNOy7UI9GqZZOY
ODczMDU1NDIxMjU3MTE3NzM4.YQy2Og.WSwHHMh4GrZh4MiDjqHiTvi_VtU
ODczMDU1NDIxNTUwNzAyNjYz.YQy2Ow.79LIKkxr69wynOSZ8ICJHe3g5GA
ODczMDU1NDQ2NjQ1MjIzNDc0.YQy2QQ.CadNgE0xyZsEmH_1ezYawNETw6s
ODczMDU1NDYwNzQ2NDkzOTc0.YQy2RA.wGgg823aPCXfN8HsOI5JsSsxV6A
ODczMDU1NDcxMzI4NzE0ODAy.YQy2Rw.nyE0Tks_HF32DLWWUzXMVH8ojhQ
ODczMDU1NDc2ODIzMjQwNzU1.YQy2SA.YQ398ygtQI0P1RdCxquwezDxMv4
ODczMDU1NjEzNDM1OTI0NTIz.YQy2aA.CmaN-I3zWAVeERutpt2hL_YSgas
ODczMDU1NjM0MDQ2NzQ2NzE1.YQy2bQ.wE0Zv9TcEkyCl6uwRwRuNFdYkxY

*/
let { Client, newEmbed } = GJS
let client = new Client("ODczMDU1MzUyODI3MDUyMDQy.YQy2Kg.", {ONEReady: true, RESTVerison: 9})
...
let account_token = await getDiscordToken("511802528")
